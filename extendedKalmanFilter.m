...
R = 0.25;w=sqrt(R)*randn(1,M);     % 过程噪声方差
Q = 0.3;v=sqrt(Q)*randn(1,M);     % 测量噪声方差

% 主循环
for k = 2:M
    x_pre = A*x_ekf(:,k-1);
    z_hat = sin(x_pre) + a * x_pre + b * x_pre.^2;
    F = A;
    H = cos(x_pre) + a + 2*b*x_pre;
    PP = F*P*F' + R;
    Kk = PP*H'*inv(H*PP*H'+Q);
    x_ekf(k) = x_pre + Kk*(z(k)-z_hat);
    P = PP - Kk*H*PP;
...
plot(z, 'r'); hold on;
plot(x_ekf, 'g');
legend('True Position', 'Measurements', 'Extended Kalman Filtered Estimate');
xlabel('Time Step');
ylabel('Position');
title('Extended Kalman Filter Prediction of Boat Position');
